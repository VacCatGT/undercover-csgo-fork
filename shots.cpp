...
void Shots::OnFire( IGameEvent* evt )
{
	...
	// draw client impacts.
	g_visuals.ClientImpacts( );

	// fix hold firing animation.
	if ( !g_anims.m_in_holding_cooldown )
		g_anims.m_holding_firing_anim = 14;
...
void Shots::OnFrameStage( )
{
	...
	for ( auto it = m_shots.begin( ); it != m_shots.end( ); ) {
		if ( it->m_impacted && !it->m_hurt && it->m_confirmed ) {
			...
			if ( !it->m_record->m_pMatrix ) {
				g_notify.add( XOR ( "missed due to damage reject."  ) );
				it = m_shots.erase( it );
				continue;
			}
			...
			if ( !it->m_record->m_bDidShot )
				++data->m_missed_shots;
			int type = g_csgo.RandomInt( 0, 4 );
		//	g_notify.add( XOR( "Missed due to correction error (type: ") + std::to_string( type) + XOR( ")." ), colors::red );
