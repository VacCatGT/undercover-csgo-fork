...
//}
//
//void UTIL_TraceRay( const Ray& ray, unsigned int mask, Entity* ignore, int collisionGroup, CGameTrace* ptr )
//{
//	CTraceFilterSimple_game traceFilter;
//	traceFilter.m_pass_ent1 = ( Entity* )ignore;
//	traceFilter.m_collision_group = collisionGroup;
//
//	g_csgo.m_engine_trace->TraceRay( ray, mask, ( ITraceFilter* )&traceFilter, ptr );
//}
//
////-----------------------------------------------------------------------------
...
////-----------------------------------------------------------------------------
//void TracePlayerBBoxForGround( const vec3_t& start, const vec3_t& end, const vec3_t& minsSrc, const vec3_t& maxsSrc, Entity* player, unsigned int fMask, int collisionGroup, CGameTrace& pm )
//{
//	Ray ray;
//	vec3_t mins, maxs;
//
//	float fraction = pm.m_fraction;
//	vec3_t endpos = pm.m_endpos;
//
...
//	if ( pm.m_entity && pm.m_plane.m_normal[ 2 ] >= 0.7 )
//	{
...
//CGameMovement::~CGameMovement( void )
//{
//	if ( !g_bIsExiting && g_csgo.m_engine_trace )
//	{
//		g_csgo.m_engine_trace->FreeTraceListData( m_pTraceListData );
//	}
//}
//
...
//// -----------------------------------
//
//void CGameMovement::ProcessMovement( Player* basePlayer, CMoveData* moveData )
//{
//	float storedFrametime = g_csgo.m_globals->m_frametime;
//
//	m_nTraceCount = 0;
//
//	g_csgo.m_globals->m_frametime *= basePlayer->m_flLaggedMovementValue( );
//
...
//
//	g_csgo.m_globals->m_frametime = storedFrametime;
//
//	m_bProcessingMovement = false;
//}
//
//void CCSGameMovement::ProcessMovement( Player* basePlayer, CMoveData* moveData )
//{
...
//}
//
//void CGameMovement::StartTrackPredictionErrors( Player* basePlayer )
//{
//	if ( basePlayer->IsBot( ) )
//		return;
//
...
//}
//
//void CGameMovement::FinishTrackPredictionErrors( Player* basePlayer )
//{
//	if ( basePlayer->IsBot( ) )
//		return;
//}
//
//const vec3_t& CGameMovement::GetPlayerMins( bool ducked ) const
//{
//	if ( ducked )
//		// (*(g_csgo.m_gamerules + 120))() + 0x14;
//		return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMin;
//
//	// (*(g_csgo.m_gamerules + 120))() + 0xC;
//	return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin;
//}
//
//const vec3_t& CGameMovement::GetPlayerMaxs( bool ducked ) const
//{
//	if ( ducked )
//		// (*(g_csgo.m_gamerules + 120))() + 0x30;
//		return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMax;
//
//	// (*(g_csgo.m_gamerules + 120))() + 0x18;
//	return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMax;
//}
//
//const vec3_t& CGameMovement::GetPlayerViewOffset( bool ducked ) const
//{
//	if ( ducked )
//		// (*(g_csgo.m_gamerules + 120))() + 0x3C;
//		return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckView;
//
//	// (*(g_csgo.m_gamerules + 120))() + 0;
//	return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vView;
//}
//
...
//}
//
//Player* CGameMovement::GetMovingPlayer( ) const
//{
//	if ( m_bProcessingMovement )
//		return player;
//
...
//}
//
//void CGameMovement::UnblockPusher( Player* pPlayer, Entity* pPusher )
//{
//	// TODO
//}
//
//void CCSGameMovement::UnblockPusher( Player* pPlayer, Entity* pPusher )
//{
//	// TODO
//}
//
...
//	else
//		m_pTraceListData = g_csgo.m_engine_trace->AllocTraceListData( );
//
//	if ( moveData->m_nPlayerHandle == -1 )
//		return;
//
//	Player* basePlayer = ( Player* )moveData->m_nPlayerHandle.Get( );
//
//	// ClearBounds inlined and optimized (doesn't use 99999 and -99999, but rather LOVAL/HIVAL constants)
//	vec3_t moveMins = { FLT_MIN, FLT_MIN, FLT_MIN };//{ (float)0xFF7FFFFF, (float)0xFF7FFFFF, (float)0xFF7FFFFF };
//	vec3_t moveMaxs = { FLT_MAX, FLT_MAX, FLT_MAX };//{ (float)0x7F7FFFFF, (float)0x7F7FFFFF, (float)0x7F7FFFFF };
//
//	vec3_t start = moveData->_m_vecAbsOrigin;
//	float radius = ( ( moveData->m_vecVelocity_.length( ) + moveData->_m_flMaxSpeed ) * g_csgo.m_globals->m_frametime ) + 1.0f;
//
//	vec3_t playerMins = GetPlayerMins( false );
//	vec3_t playerMaxs = GetPlayerMaxs( false );
//
//	vec3_t bloat = { radius, radius, radius + basePlayer->GetStepSize( ) };
//	
//	math::AddPointToBounds( start + playerMaxs + bloat, moveMins, moveMaxs );
//	math::AddPointToBounds( start + playerMaxs - bloat, moveMins, moveMaxs );
//
//	// (*(*enginetrace + 28))(&moveMins, &moveMaxs, *(gameMovement + 0xE4C));
//	g_csgo.m_engine_trace->SetupLeafAndEntityListBox( moveMins, moveMaxs, m_pTraceListData );
//}
//
//vec3_t CGameMovement::GetPlayerMins( )
//{
//	// (*(**(this + 4) + 0x474))()
//	if ( player->m_iObserverMode( ) )
//		// (*(g_csgo.m_gamerules + 120))() + 72;
//		return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vObsHullMin;
//
//	// ( *(*(v1 + 4) + 0x3034) )
//	if ( player->m_bDucking( ) )
//		// (*(v3 + 120))(g_csgo.m_gamerules) + 36
//		return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMin;
//
//	// (*(v3 + 120))(g_csgo.m_gamerules) + 12
//	return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin;
//}
//
//vec3_t CGameMovement::GetPlayerMaxs( )
//{
//	// (*(**(this + 4) + 0x474))()
//	if ( player->m_iObserverMode( ) )
//		// (*(g_csgo.m_gamerules + 120))() + 84;
//		return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vObsHullMax;
//
//	// ( *(*(v1 + 4) + 0x3034) )
//	if ( player->m_bDucking( ) )
//		// (*(v3 + 120))(g_csgo.m_gamerules) + 48
//		return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMax;
//
//	// (*(v3 + 120))(g_csgo.m_gamerules) + 24
//	return ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMax;
//}
//
//void CGameMovement::TracePlayerBBox( const vec3_t& rayStart, const vec3_t& rayEnd, int fMask, int collisionGroup, CGameTrace& tracePtr )
//{
//	Ray ray;
//	m_nTraceCount++;
//
//	vec3_t playerMaxs = GetPlayerMaxs( );
//	vec3_t playerMins = GetPlayerMins( );
//
...
//		g_csgo.m_engine_trace->TraceRay( ray, fMask, filter, &tracePtr );
//	else
//		g_csgo.m_engine_trace->TraceRayAgainstLeafAndEntityList( ray, m_pTraceListData, fMask, filter, &tracePtr );
//
//	UnlockTraceFilter( filter );
//}
//
//unsigned int CGameMovement::PlayerSolidMask( bool brushOnly, Player* testPlayer /*= nullptr;*/ )
//{
//	if ( brushOnly )
//		return 0x1400B;
//
...
//
////dangerzone correct
//unsigned int CCSGameMovement::PlayerSolidMask( bool brushOnly, Player* testPlayer )
//{
//	bool isBot = true;
//
//	if ( player != nullptr )
...
//		// Calls Player's 148th function
//		v9 = player->GetPhysicsSolidMaskForEntity( );
//
...
//{
//	const float maxspeed = g_csgo.sv_maxspeed->GetFloat( ) * factor;
//
...
//
//	math::VectorNormalizeFast( forward );
//	math::VectorNormalizeFast( right );
//
//	vec3_t wishVel;
//	for ( int i = 0; i < 3; ++i )
...
//
//		float friction = g_csgo.sv_friction->GetFloat( ) * player->GetSurfaceFriction( );
//
//		float drop = control * friction * g_csgo.m_globals->m_frametime;
//
...
//
//	vec3_t temp;
//	VectorMA( mv->_m_vecAbsOrigin, g_csgo.m_globals->m_frametime, mv->m_vecVelocity_, temp );
...
//{
//	// (*(*ecx->player + 0x474))();
//	int observerMode = player->m_iObserverMode( );
//
//	if ( observerMode == OBS_MODE_IN_EYE || observerMode == OBS_MODE_CHASE )
//	{
//		// (*(*ecx->player + 0x478))();
//		Player* observerTarget = ( Player* )player->GetObserverTarget( );
//
...
//
//			float drop = spd * friction * g_csgo.m_globals->m_frametime;
//
//			float newspeed = fmaxf( spd - drop, 0.f );
//
//			newspeed /= spd;
...
//		// Calls Player's 337th index with surfacedata (player + 35A0)
...
//	float newtime = ( g_csgo.m_globals->curtime - ( player->GetAutomoveTargetTime( ) - dt ) ) / dt;
//	newtime = clamp( newtime, 0.0f, 1.0f );
//	float fuck = powf( newtime, 0.6214906f );
//	vec3_t origin = ( player->GetAutomoveTargetEnd( ) - player->GetAutoMoveOrigin( ) ) * fuck + player->GetAutoMoveOrigin( );
...
//
//		Vector2D dest;
//		vec3_t delta = player->GetAutomoveTargetEnd( ) - player->GetAutoMoveOrigin( );
//
...
//			{
//				float newvelocitymodifier = velocitymodifier + ( g_csgo.m_globals->m_frametime * 0.4f );
...
////-----------------------------------------------------------------------------
//void CGameMovement::PerformFlyCollisionResolution( CGameTrace& pm, vec3_t& move )
//{
//	vec3_t base;
//	float vel;
//	float backoff;
...
//			backoff = 1;
//
//		ClipVelocity( mv->m_vecVelocity_, pm.m_plane.m_normal, mv->m_vecVelocity_, backoff );
//	}
//	break;
//
//	default:
...
//
//	// stop if on ground
//	if ( pm.m_plane.m_normal[ 2 ] > 0.7 )
//	{
//		base.Init( );
//		if ( mv->m_vecVelocity_[ 2 ] < sv_gravity.GetVar( )->GetFloat( ) * g_csgo.m_globals->m_frametime )
//		{
...
//
//		// Con_DPrintf("%f %f: %.0f %.0f %.0f\n", vel, trace.m_fraction, ent->velocity[0], ent->velocity[1], ent->velocity[2] );
//
...
//		else
//		{
//			VectorScale( mv->m_vecVelocity_, ( 1.0 - pm.m_fraction ) * g_csgo.m_globals->m_frametime * 0.9, move );
//			PushEntity( move, &pm );
//		}
...
//}
//
//float CGameMovement::CalcRoll( const ang_t& angles, const vec3_t& velocity, float rollangle, float rollspeed )
//{
//	vec3_t forward, right, up;
//	AngleVectors( ( ang_t )angles, &forward, &right, &up );
//
...
//
//	float timemultiplier = g_csgo.m_globals->interval_per_tick * viewPunchDecay;
//	timemultiplier = expf( -timemultiplier );
//	ang_t viewpunch = player->GetViewPunch( );
//	ang_t newpunch = viewpunch * timemultiplier;
//
...
//		{
//			vec3_t flatVelocity = { mv->m_vecVelocity_.x, mv->m_vecVelocity_.y, 0.0f };
//			float curspeed = VectorNormalize( flatVelocity );
//
//			vec3_t flatForward = { m_vecForward.x, m_vecForward.y, 0.0f };
...
//					IPhysicsObject* physObj = tr.m_entity->VPhysicsGetObject( );
//
//					if ( physObj )
//					{
...
//
//					// !((*(*physicsObj + 76))() & 4) )
//					if ( !tr.m_entity || tr.m_entity->CanWaterJump( ) )
//					{
...
//						vec3_t waterjumpvel;
//						VectorMA( vec3_t( 0, 0, 0 ), -50.0f, tr.m_plane.m_normal, waterjumpvel );
...
//							TracePlayerBBox( start, end, mask, COLLISION_GROUP_PLAYER_MOVEMENT, tr );
//
//							if ( tr.m_fraction < 1.f && tr.m_plane.m_normal.z >= 0.7 )
//							{
...
//		newspeed = speed - g_csgo.m_globals->curtime * speed * sv_friction.GetVar( )->GetFloat( ) * player->GetSurfaceFriction( );
//
//		if ( newspeed < 0.1f )
//			newspeed = 0.0f;
//
...
//			float accelspeed = sv_accelerate.GetVar( )->GetFloat( ) * wishspeed * g_csgo.m_globals->curtime * player->GetSurfaceFriction( );
//
...
//
//	vec3_t dest;
//	VectorMA( mv->_m_vecAbsOrigin, g_csgo.m_globals->curtime, mv->m_vecVelocity_, dest );
//
//	CGameTrace tr;
...
//		{
//			float stepDist = tr.m_endpos.z - mv->_m_vecAbsOrigin.z;
//			mv->m_outStepHeight += stepDist;
//
//			mv->_m_vecAbsOrigin = tr.m_endpos;
//
//			goto jmp;
//		}
//
...
//	{
//		player->SetWaterJumpTime( player->GetWaterJumpTime( ) - g_csgo.m_globals->m_frametime * 10000.f );
//
...
//}
//
//void CGameMovement::AirAccelerate( vec3_t& wishdir, float wishspeed, float accel )
//{
...
//		if ( v3 > 0.f )
//		{
//			float v7 = fminf( wishspeed * accel * g_csgo.m_globals->m_frametime * player->GetSurfaceFriction( ), v3 );
//
...
//	}
//	vec3_t boxmin = { -12.0f, -12.0f, 0.0f };
//	vec3_t boxmax = { 12.0f, 12.0f, 3.0f };
//	vec3_t boxmin2 = { -10.0f, -10.0f, 0.0f };
//	vec3_t boxmax2 = { 10.0f, 10.0f, 24.0f };
//
//	if ( mv->m_flForwardMove <= 0.0f || !( mv->m_nButtons & IN_DUCK ) )
//		return;
//
//	vec3_t vel = mv->m_vecVelocity_;
//
//	float speed = vel.length( );
//
//	float speedclamp = fmaxf( speed * 0.12f, 12.0f );
//
//	vec3_t* origin = cplayer->GetAbsOrigin( );
//
//	CGameTrace tr;
//	UTIL_TraceLine( *origin, *origin + vec3_t( 0.0f, 0.0f, -14.0f ), 0x2000B, cplayer, COLLISION_GROUP_PLAYER_MOVEMENT, &tr );
//
//
//	vec3_t end;
...
//	if ( debug )
//	{
//		if ( tr2.m_fraction == 1.0f && Interfaces::DebugOverlay )
//			Interfaces::DebugOverlay->AddBoxOverlay( tr2.m_endpos, boxmin2, boxmax2, angZero, 255, 0, 80, 255, 6.0f );
//
//		if ( tr.m_fraction != 1.0f )
//		{
//			vec3_t endpos = *cplayer->GetAbsOrigin( );
//			endpos.z -= 14.0f;
...
//		return;
//
//	vec3_t pos1 = *cplayer->GetAbsOrigin( );
//	pos1.z += 2.0f;
//
//	vec3_t pos2 = pos2;
...
//	filter.m_pPassEnt1 = ( Entity* )cplayer;
...
//	if ( debug )
//	{
//		if ( tr3.m_fraction != 1.0f )
//		{
//			if ( Interfaces::DebugOverlay )
//				Interfaces::DebugOverlay->AddBoxOverlay( tr3.m_endpos, boxmin, boxmax, angZero, 255, 0, 0, 128, 6.0f );
//			return;
//		}
//		if ( Interfaces::DebugOverlay )
//			Interfaces::DebugOverlay->AddBoxOverlay( tr3.m_endpos, boxmin, boxmax, angZero, 0, 255, 0, 255, 6.0f );
//	}
//	else if ( tr3.m_fraction != 1.0f )
//	{
//		return;
//	}
//
//	float v47 = ( tr2.m_fraction * v107 ) + 1.5f;
//	CGameTrace tr4;
//
//	vec3_t startnew;
//	startnew.z = pos1.z;
//	startnew.x = tr2.m_endpos.x + ( v107 * v47 );
//	startnew.y = tr2.m_endpos.y + ( v108 * v47 );
//	vec3_t endnew;
//	endnew.x = startnew.x;
//	endnew.y = startnew.y;
...
//	{
//		vec3_t newend2;
//		newend2 = tr4.m_endpos;
//		newend2.z += 3.0f;
//
//		CViewVectors* vectors = GetGamerules( )->GetViewVectors( );
//		CGameTrace tr5;
//		UTIL_TraceHull( tr4.m_endpos, newend2, vectors->m_vHullMin, vectors->m_vHullMax, PlayerSolidMask( ), ( ITraceFilter* )&filter, tr5 );
//
//		bool v100;
//
//		if ( tr5.startsolid || tr5.allsolid || ( v100 = true, tr5.m_fraction != 1.0f ) )
//			v100 = false;
//
//		vec3_t newend3 = tr4.m_endpos;
//		newend3.z += 3.0f;
//		CGameTrace tr6;
//
//		UTIL_TraceHull( tr4.m_endpos, newend3, vectors->m_vDuckHullMin, vectors->m_vDuckHullMax, PlayerSolidMask( ), ( ITraceFilter* )&filter, tr6 );
//
//		if ( tr6.startsolid || tr6.allsolid || tr6.m_fraction != 1.0f )
//		{
//			vec3_t end4;
//			//end4.x = tr4.m_endpos.x + (tr.m_endpos.x)
//
//			//FIXME: FINISH ME
//		}
//	}
...
//
//	Entity* movingplayer = GetMovingPlayer( );
...
//				{
//					vec3_t someVector = vecZero;
//					vec3_t destVector = vecZero;
//					for ( int i = 1; i <= MAX_PLAYERS; i++ ) //VALVE BUG: this is set to 0 and < MAX_PLAYERS which is wrong!!
//					{
//						Entity* playerindex = Interfaces::ClientEntList->GetBaseEntity( i ); //UTIL_PlayerFromIndex is the original function
//						if ( playerindex && playerindex->IsPlayer( ) && movingplayer != playerindex && playerindex->GetAliveVMT( ) && playerindex->GetMaxFallVelocity( ) != 0.0f )
//						{
//							vec3_t movingplayerorigin = *playerindex->GetAbsOrigin( );
//							vec3_t vecDelta = movingplayerorigin - *movingplayer->GetAbsOrigin( );
...
//								}
//
//								vec3_t normalized = movingplayerorigin;
//								VectorNormalizeFast( normalized );
//								destVector = someVector;
//								vec3_t newpos = normalized * maxspd;
//
...
//}
//
//void CGameMovement::Accelerate( vec3_t& wishdir, float wishspeed, float accel )
//{
//	if ( CanAccelerate( ) )
//	{
...
//		if ( v6 > 0.f )
//		{
//			float v7 = fminf( g_csgo.m_globals->m_frametime * accel * fmaxf( wishspeed, 250.f ) * player->GetSurfaceFriction( ), v6 );
//
//			mv->m_vecVelocity_ += wishdir * v7;
//		}
//	}
...
//void CCSGameMovement::Accelerate( vec3_t& wishdir, float wishspeed, float accel )
//{
//	if ( !CanAccelerate( ) )
//		return;
//
...
//	float v26 = ( ( g_csgo.m_globals->m_frametime * accel ) * finalwishspeed ) * surfacefriction;
//
...
//	}
//
//	float v30 = fminf( ( ( g_csgo.m_globals->m_frametime * accel ) * finalwishspeed ) * surfacefriction, v60 );
//
//	if ( move_player )
//	{
//		float boostdelta = move_player->GetHealthShotBoostExpirationTime( ) - g_csgo.m_globals->curtime;
...
//	}
//
//	//float v24 = fminf(g_csgo.m_globals->m_frametime * accel * backupfinalwishspeed * player->GetSurfaceFriction(), v46);
//
//	mv->m_vecVelocity_ += wishdir * v30;
//
//	// movss   xmm0, dword ptr [ecx+0A2C0h]
//	if ( m_pCSPlayer->GetGroundAccelLinearFracLastTime( ) != g_csgo.m_globals->curtime )
//	{
//		m_pCSPlayer->SetGroundAccelLinearFracLastTime( g_csgo.m_globals->curtime );
//	}
//
...
//				mv->somefloat = g_csgo.m_globals->curtime;
//				return;
//			}
//		}
//	}
...
//	float v37 = g_csgo.m_globals->curtime - mv->somefloat;
//
//	if ( v37 > 0.35f )
//	{
//		mv->m_vecOldVelocity = mv->m_vecVelocity_;
...
//
//	vec3_t temp = {
//		mv->m_vecVelocity_.x * g_csgo.m_globals->m_frametime + mv->_m_vecAbsOrigin.x,
//		mv->m_vecVelocity_.y * g_csgo.m_globals->m_frametime + mv->_m_vecAbsOrigin.y,
...
//
//	vec3_t temp = tr.m_endpos;
//
//	TracePlayerBBox( temp, end, PlayerSolidMask( ), COLLISION_GROUP_PLAYER_MOVEMENT, tr );
//
//	if ( tr.m_fraction > 0.f && tr.m_fraction < 1.f && !tr.startsolid && tr.m_plane.m_normal.z >= 0.7f )
//	{
//		float unk = fabs( mv->_m_vecAbsOrigin.z - tr.m_endpos.z );
//
//		if ( unk > 0.015625f )
//			mv->_m_vecAbsOrigin = tr.m_endpos;
//	}
...
//	{
//		tickInterval = static_cast< int >( 0.3f / g_csgo.m_globals->interval_per_tick );
//		break;
//	}
//
...
//			tickInterval = 1;
//		else if ( g_csgo.m_globals->maxClients == 1 )
//			tickInterval = static_cast< int >( 0.2f / g_csgo.m_globals->interval_per_tick );
//		else
//			tickInterval = static_cast< int >( 1.f / g_csgo.m_globals->interval_per_tick );
//		break;
//	}
//
...
//	{
//		tickInterval = static_cast< int >( 0.5f / g_csgo.m_globals->interval_per_tick );
//		break;
//	}
//
//	default:
...
//		grav = 1.0f;
//
//	vec3_t basevel = player->GetBaseVelocity( );
//
//	mv->m_vecVelocity_.z -= ( ( sv_gravity.GetVar( )->GetFloat( ) * grav ) * 0.5f ) * g_csgo.m_globals->m_frametime;
//	mv->m_vecVelocity_.z += basevel.z * g_csgo.m_globals->m_frametime;
//
//	basevel.y = basevel.x;
...
//			grav = 1.0f;
//
//		mv->m_vecVelocity_.z -= sv_gravity.GetVar( )->GetFloat( ) * grav * g_csgo.m_globals->m_frametime * 0.5f;
//		CheckVelocity( );
//	}
//}
//
...
//		vec3_t basevel = player->GetBaseVelocity( );
//
//		mv->m_vecVelocity_.z -= sv_gravity.GetVar( )->GetFloat( ) * grav * g_csgo.m_globals->m_frametime;
//		mv->m_vecVelocity_.z += basevel.z * g_csgo.m_globals->m_frametime;
//
//		basevel.z = 0.f;
//		player->SetBaseVelocity( basevel );
...
//	if ( player->GetWaterJumpTime( ) != 0.0f )
//	{
//		player->SetWaterJumpTime( player->GetWaterJumpTime( ) - g_csgo.m_globals->m_frametime );
//
//		if ( player->GetWaterJumpTime( ) < 0.0f )
...
//
//	if ( g_csgo.m_globals->maxClients == 1 )
//	{
//		// player + 0x3000
...
//	else
//	{
//		m_pCSPlayer->SetWaterJumpTime( m_pCSPlayer->GetWaterJumpTime( ) - g_csgo.m_globals->m_frametime );
//
//		if ( m_pCSPlayer->GetWaterJumpTime( ) < 0.f )
...
//		SetGroundEntity( nullptr );
//
//	if ( player->GetGroundEntity( ) != nullptr || player->GetBaseVelocity( ) != vec3_t( 0, 0, 0 ) || mv->m_vecVelocity_ != vec3_t( 0, 0, 0 ) )
//	{
//		CheckVelocity( );
//
...
//
//		vec3_t temp;
//		VectorScale( mv->m_vecVelocity_, g_csgo.m_globals->m_frametime, temp );
//
//		mv->m_vecVelocity_ -= player->GetBaseVelocity( );
//
//		vec3_t end;
...
//
//		if ( tr.m_fraction < 1.f && !tr.allsolid )
...
//}
//
//int CGameMovement::TryPlayerMove( vec3_t* firstDest /*= nullptr*/, CGameTrace* firstTrace /*= nullptr */ )
//{
//	vec3_t originalVelocity = mv->m_vecVelocity_;
//	vec3_t primalVelocity = mv->m_vecVelocity_;
//	vec3_t unknownVelocity = mv->m_vecVelocity_;
//	CGameTrace pm;
//
...
//		//  end point
//		vec3_t end = mv->_m_vecAbsOrigin + ( mv->m_vecVelocity_ * frametime );
//
...
//		}
//
//		if ( pm.m_fraction > 0.0f && pm.m_fraction < 0.0001f ) //0.000099999997f
//		{
//			pm.m_fraction = 0.0f;
//		}
//
...
//		if ( pm.m_fraction > 0.0f )
//		{
//			if ( pm.m_fraction == 1.0f )
//			{
...
//				CGameTrace stuck;
//				TracePlayerBBox( pm.m_endpos, pm.m_endpos, PlayerSolidMask( ), COLLISION_GROUP_PLAYER_MOVEMENT, stuck );
//				fVol = 1.f;
//
//				if ( stuck.allsolid || stuck.m_fraction != 1.f )
//				{
//					mv->m_vecVelocity_.Zero( );
...
//			numplanes = 0;
//			mv->_m_vecAbsOrigin = pm.m_endpos;
...
//		//  step or wall
//		if ( fabsf( pm.m_plane.m_normal.z ) < 0.0001f ) //0.000099999997f
//		{
//			pm.m_plane.m_normal.z = 0.0f;
//			blocked |= 2; // step / wall
//		}
//
//		// Reduce amount of m_flFrameTime left by total time left * fraction
//		//  that we covered.
//		frametime -= ( pm.m_fraction * frametime );
//
...
//		planes[ numplanes++ ] = pm.m_plane.m_normal;
...
//
//	if ( tr.m_fraction == 1.f || !OnLadder( tr ) )
//	{
//		//0x3208
...
//					{
//						vec3_t start = mv->_m_vecAbsOrigin;
//						start.z -= 6.0f;
//
//						vec3_t normalized = mv->m_vecVelocity_;
//						VectorNormalizeFast( normalized );
//						normalized *= 24.f;
//
//						vec3_t end = mv->_m_vecAbsOrigin - normalized;
//						TracePlayerBBox( start, end, LadderMask( ) & 0xFFFEFFFF, COLLISION_GROUP_PLAYER_MOVEMENT, tr );
//
//						if ( tr.m_fraction == 1.f && OnLadder( tr ) && tr.m_plane.m_normal.z != 1.f )
//						{
//							player->SetMoveType( MOVETYPE_LADDER );
//							player->SetMoveCollide( MOVECOLLIDE_DEFAULT );
//
//							vec3_t ladderNormal = player->GetVecLadderNormal( );
//
//							if ( tr.m_plane.m_normal != ladderNormal )
//							{
//								player->SetVecLadderNormal( tr.m_plane.m_normal );
//							}
//
//							mv->m_vecVelocity_.x = 0.f;
//							mv->m_vecVelocity_.y = 0.f;
...
//							if ( player->GetMoveType( ) != MOVETYPE_LADDER )
//								// *(float*)(player + 0x3204)
//								player->SetTimeNotOnLadder( g_csgo.m_globals->curtime + 0.2f );
//
//							player->SetMoveType( MOVETYPE_LADDER );
//							player->SetMoveCollide( MOVECOLLIDE_DEFAULT );
//
//							if ( tr.m_plane.m_normal != player->GetVecLadderNormal( ) )
//							{
//								player->SetVecLadderNormal( tr.m_plane.m_normal );
//							}
//
//							vec3_t floor = mv->_m_vecAbsOrigin;
//							floor.z += ( GetPlayerMins( ).z - 1.f );
//
//							bool onFloor = g_csgo.m_engine_trace->GetPointContents( floor ) == CONTENTS_SOLID || player->GetGroundEntity( ) != nullptr;
//
...
//							if ( mv->m_nButtons & IN_JUMP )
//							{
//								if ( g_csgo.m_globals->curtime >= player->GetTimeNotOnLadder( ) )
//								{
//									player->SetMoveType( MOVETYPE_WALK );
//									player->SetMoveCollide( MOVECOLLIDE_DEFAULT );
//
//									mv->m_vecVelocity_ = tr.m_plane.m_normal * 270.f;
//									return true;
//								}
//							}
//							else
...
//								float v56 = tr.m_plane.m_normal.x;
...
//								float v60 = tr.m_plane.m_normal.z;
//								float v61 = v58;
//								float v62 = ( m_vecRight.y * rightspeed ) + v59;
//								float v63 = ( m_vecRight.z * rightspeed ) + v57;
//								float v64 = ( tr.m_plane.m_normal.z * 0.f ) - tr.m_plane.m_normal.y;
//								float v65 = tr.m_plane.m_normal.y;
//								float v66 = tr.m_plane.m_normal.x - ( tr.m_plane.m_normal.z * 0.f );
//								float v67 = ( tr.m_plane.m_normal.y * 0.f ) - ( tr.m_plane.m_normal.x * 0.f );
...
//								float v77 = ( ( v65 * v74 ) + v76 ) + ( tr.m_plane.m_normal.z * v75 );
//								float v98 = ( ( v65 * v74 ) + v76 ) + ( tr.m_plane.m_normal.z * v75 );
//
...
//								{
//									mv->m_vecVelocity_ += ( tr.m_plane.m_normal * 200.f );
//								}
//							}
//						}
//					}
...
//}
//
//bool CGameMovement::OnLadder( CGameTrace& pm )
//{
...
//
////dangerzone correct
//bool CCSGameMovement::OnLadder( CGameTrace& trace )
//{
//	if ( trace.m_plane.m_normal.z != 1.f )
//	{
...
//}
//
//int CGameMovement::GetWaterContentsForPointCached( const vec3_t& point, int slot )
//{
...
//	{
//		m_CachedGetPointContents[ idx ][ slot ] = g_csgo.m_engine_trace->GetPointContents( point, MASK_WATER );
...
//}
//
//void CGameMovement::PushEntity( vec3_t& push, CGameTrace* pTrace )
//{
//	vec3_t end;
//
...
//}
//
//int CGameMovement::ClipVelocity( vec3_t& in, vec3_t& normal, vec3_t& out, float overbounce )
//{
//	int blocked = 0;
...
//		m_flWaterEntryTime = g_csgo.m_globals->curtime;
//
//	return player->GetWaterLevel( ) > 1;
//}
//
//void CGameMovement::GetWaterCheckPosition( int waterLevel, vec3_t* pos )
//{
...
//		return;
//
//	vec3_t point = mv->_m_vecAbsOrigin;
//	point.z -= 2.0f;
//
...
//		jmp:
//			SetGroundEntity( &tr );
//			if ( bMoveToEndPos && !tr.startsolid && tr.m_fraction > 0.f && tr.m_fraction < 1.f )
//				mv->_m_vecAbsOrigin = tr.m_endpos;
//		}
//	}
//}
...
//	if ( player->GetHealth( ) <= 0 )
//	{
//		//  v24 = (*(g_csgo.m_gamerules + 120))();
//		// (*(v23 + 680))(v2->player, v24 + 96);
//		player->SetViewOffset( ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDeadViewHeight );
//	}
//
...
//		vec3_t velocity = m_pCSPlayer->GetVelocity( );
//
...
//
//	// (*(*player + 0x474))() != 6 )
//	if ( ( player->GetFlags( ) & 0x50 || player->GetHealth( ) <= 0 ) && player->m_iObserverMode( ) != OBS_MODE_ROAMING )
//	{
//		mv->m_flForwardMove = 0.f;
//		mv->m_flSideMove = 0.f;
//		mv->m_flUpMove = 0.f;
...
//	else
//	{
//		ang_t angles = mv->m_vecAngles + player->GetPunch( );
//
//		if ( player->GetMoveType( ) == MOVETYPE_ISOMETRIC || player->GetMoveType( ) == MOVETYPE_NOCLIP )
//		{
//			mv->m_vecAngles.z = 0.f;
...
//	}
//
//	if ( player->GetHealth( ) <= 0 && player->m_iObserverMode( ) == OBS_MODE_DEATHCAM )
//	{
//		player->SetViewOffset( ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDeadViewHeight );
//	}
//
//	mv->m_vecAngles.y = AngleNormalize( mv->m_vecAngles.y );
//
//	if ( player->m_iObserverMode( ) == OBS_MODE_NONE )
//	{
//		if ( player->GetMoveType( ) != MOVETYPE_LADDER )
//		{
//			int nLevels = 0;
//			Entity* pCurGround = player->GetGroundEntity( );
...
//	{
//		player->SetDuckTimeMsecs( ( float )player->GetDuckTimeMsecs( ) - ( g_csgo.m_globals->m_frametime * 1000.f ) );
//
//		if ( player->GetDuckTimeMsecs( ) < 0 )
...
//	{
//		player->SetDuckJumpTimeMsecs( ( float )player->GetDuckJumpTimeMsecs( ) - ( g_csgo.m_globals->m_frametime * 1000.f ) );
//
//		if ( player->GetDuckJumpTimeMsecs( ) < 0 )
...
//	{
//		player->SetJumpTimeMsecs( ( float )player->GetJumpTimeMsecs( ) - ( g_csgo.m_globals->m_frametime * 1000.f ) );
//
//		if ( player->GetJumpTimeMsecs( ) < 0 )
...
//	{
//		player->SetSwimSoundTime( player->GetSwimSoundTime( ) - ( g_csgo.m_globals->m_frametime * 1000.f ) );
//
//		if ( player->GetSwimSoundTime( ) < 0.f )
...
//	if ( m_pCSPlayer->GetStamina( ) > 0.f )
//	{
//		float v4 = g_csgo.m_globals->m_frametime * sv_staminarecoveryrate.GetVar( )->GetFloat( );
//
...
//
//	Entity* groundEntityPtr = player->GetGroundEntity( );
//
...
//					{
//						FinishUnduckJump( tr );
//						player->SetDuckJumpTimeMsecs( ( ( ( ( 1.f - tr.m_fraction ) * tr.m_fraction ) * 200.f ) + 800.f ) );
//					}
//				}
//			}
//			else
...
//
//			if ( tr.m_fraction < 1.f )
//				player->SetDuckJumpTimeMsecs( ( ( ( 1.f - tr.m_fraction ) * 200.f ) + 800.f ) );
//		}
//	}
//
//	if ( player->GetDuckJumpTimeMsecs( ) <= 0 )
...
//					player->SetDuckTimeMsecs( 1000 );
//				}
//				else if ( player->GetDucking( ) && !player->m_bDucking( ) )
//				{
...
//			if ( CanUnduck( ) )
//			{
//				if ( player->GetDucking( ) && !player->m_bDucking( ) )
//					return;
//
...
//{
//	if ( m_pCSPlayer->GetDuckSpeed( ) <= 1.5f || !( m_pCSPlayer->GetFlags( ) & FL_DUCKING ) || ( m_pCSPlayer->GetLastDuckTime( ) + sv_timebetweenducks.GetVar( )->GetFloat( ) ) > g_csgo.m_globals->curtime )
//		return false;
//	return true;
//}
//
////dangerzone correct
//bool CCSGameMovement::CanMove( Player* ent )
//{
//	if ( ent->GetMoveType( ) != MOVETYPE_NONE )
//	{
//		if ( ent->m_iObserverMode( ) )
//			return true;
//
...
//			&& ( !( g_csgo.m_gamerules )->IsFreezePeriod( ) || ent->GetCanMoveDuringFreezePeriod( ) )
...
//{
//	if ( player->m_iObserverMode( ) != 6 && !m_bSpeedCropped )
//	{
//		if ( mv->m_nButtons & IN_DUCK || player->GetDucking( ) || player->GetFlags( ) & FL_DUCKING )
//		{
...
//		mv->m_nOldButtons &= ~IN_DUCK;
//
//	if ( player->GetHealth( ) <= 0 && !( player->m_iObserverMode( ) ) )
//	{
...
//		float v32 = g_csgo.m_globals->m_frametime * 3.f;
//
...
//			vec3_t* absorigin = player->GetAbsOrigin( );
//
//			// probably some sort of velocity lol
//			Vector2D duckingorigin = player->GetDuckingOrigin( );
//			vec3_t duckingorigin2 = { duckingorigin.x, duckingorigin.y, 0 };
//
//			if ( ( duckingorigin2 - *absorigin ).Length2DSqr( ) > 4096.f )
//			{
//				float v51;
//				float v50 = g_csgo.m_globals->m_frametime * 6.0f;
//
...
//					v57 *= 0.4f;
//
//				float v59 = g_csgo.m_globals->m_frametime * v57;
//
...
//						adjust *= 0.4f;
//
//					float v44 = g_csgo.m_globals->m_frametime * adjust;
//					float v45;
//
...
//		CGameTrace tr;
//
//		vec3_t newOrigin = mv->_m_vecAbsOrigin;
//
//		vec3_t hullSizeNormal = ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMax - ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin;
//		vec3_t hullSizeDuck = ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMax - ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMin;
//		vec3_t delta = hullSizeNormal - hullSizeDuck;
//		newOrigin -= delta;
//
//		vec3_t groundCheck = newOrigin;
//		groundCheck.z -= player->GetStepSize( );
//
//		UTIL_TraceHull( newOrigin, groundCheck, ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin, ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMax, /*CGameMovement::*/ PlayerSolidMask( ), player, COLLISION_GROUP_PLAYER_MOVEMENT, tr );
//
//		if ( !tr.startsolid && tr.m_fraction != 1.f )
//		{
//			m_pCSPlayer->SetDuckUntilOnGround( false );
//
//			if ( CanUnduck( ) )
...
//	{
//		for ( int i = 0; i < 3; ++i )
//			newOrigin[ i ] += ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMin[ i ] - ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin[ i ];
//	}
//	else
//	{
//		vec3_t hullSizeNormal = ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMax - ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin;
...
//
//	vec3_t viewOffset = GetPlayerViewOffset( false );
//
...
//	{
//		newOrigin += ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMin - ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin;
//	}
//	else
//	{
//		vec3_t hullSizeNormal = ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMax - ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin;
//		vec3_t hullSizeDuck = ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMax - ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMin;
//		vec3_t delta = ( hullSizeNormal - hullSizeDuck ) * -0.5f;
//
//		newOrigin += delta;
//	}
//
...
//		player->SetDucked( TRUE );
//
//		vec3_t viewOffset = GetPlayerViewOffset( true );
//		player->SetViewOffset( viewOffset );
//
//		if ( player->GetGroundEntity( ) != nullptr )
//		{
//			vec3_t origin = mv->_m_vecAbsOrigin;
//
//			for ( int i = 0; i < 3; ++i )
//				origin[ i ] -= ( ( g_csgo.m_gamerules )->GetViewVectors( )->m_vDuckHullMin[ i ] - ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin[ i ] );
//
//			mv->_m_vecAbsOrigin = origin;
//		}
//		else
...
//	player->SetDucking( FALSE );
//	player->SetDucked( TRUE );
//	player->SetLastDuckTime( g_csgo.m_globals->curtime );
//	player->AddFlag( FL_DUCKING | FL_ANIMDUCKING );
//
//	FixPlayerCrouchStuck( true );
//	CategorizePosition( );
...
//	}
//
//	bool duckedBackup = player->m_bDucking( );
//	player->SetDucked( FALSE );
...
//
//	return ( !tr.startsolid && tr.m_fraction == 1.f );
//}
//
////dangerzone correct
...
//	//dangerzone update:
//	CTraceFilterForPlayerHeadCollision filter;
//	filter.m_pPassEnt1 = ( Entity* )player;
//	filter.m_pSkip = ( Entity* )player;
//	filter.m_Unknown = 0;
//	filter.m_iCollisionGroup = COLLISION_GROUP_PLAYER_MOVEMENT;
//
//	UTIL_TraceHull( mv->_m_vecAbsOrigin, newOrigin, ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin, ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMax, PlayerSolidMask( ), ( ITraceFilter* )&filter, trace );
//
//	if ( trace.startsolid || trace.m_fraction != 1.f )
//		return false;
//
//	return true;
//}
...
//}
//
//bool CGameMovement::CanUnduckJump( CGameTrace& tr )
//{
//	vec3_t end = mv->_m_vecAbsOrigin;
//	end.z -= 36.f;
//
//	TracePlayerBBox( mv->_m_vecAbsOrigin, end, PlayerSolidMask( ), COLLISION_GROUP_PLAYER_MOVEMENT, tr );
//
...
//		return false;
//
//	end.z = mv->_m_vecAbsOrigin.z - ( tr.m_fraction * 36.f );
//
//	bool saveducked = player->m_bDucking( );
//
//	player->SetDucked( FALSE );
//
//	CGameTrace trace;
...
//}
//
//void CGameMovement::FinishUnduckJump( CGameTrace& tr )
//{
//	vec3_t newOrigin = mv->_m_vecAbsOrigin;
//
//	vec3_t hullSizeNormal = ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMax - ( g_csgo.m_gamerules )->GetViewVectors( )->m_vHullMin;
...
//
//	vec3_t mins_duck = GetPlayerMins( true );
//	vec3_t mins_stand = GetPlayerMins( false );
//
//	float fMore = ( mins_duck.z - mins_stand.z );
//
//	vec3_t view_duck = GetPlayerViewOffset( true );
//	vec3_t view_stand = GetPlayerViewOffset( false );
//
//	vec3_t temp = player->GetViewOffset( );
...
//	{
//		vec3_t temp = mv->_m_vecAbsOrigin;
//
//		for ( int i = 0; i < 36; ++i )
//		{
//			vec3_t orig = mv->_m_vecAbsOrigin;
//			orig.z += direction;
...
//
//int GetRandomStuckOffsets( Player* pPlayer, vec3_t& offset )
//{
...
////			server -
////-----------------------------------------------------------------------------
//void ResetStuckOffsets( Player* pPlayer )
//{
...
//}
//
//void CGameMovement::CategorizeGroundSurface( CGameTrace& tr )
//{
...
//}
//
//CBaseHandle CGameMovement::TestPlayerPosition( const vec3_t& pos, int collisionGroup, CGameTrace& pm )
//{
//	++m_nTraceCount;
//
//	vec3_t maxs = GetPlayerMaxs( );
//	vec3_t mins = GetPlayerMins( );
//
//	Ray ray;
...
//	UnlockTraceFilter( filter );
//
//	if ( ( pm.contents & PlayerSolidMask( ) ) && pm.m_entity != nullptr )
//	{
//		// v11 = *(*(*v10 + 8))();
//		return pm.m_entity->GetRefEHandle( );
//	}
//
//	return ( CBaseHandle )-1;
//}
//
//void CGameMovement::SetGroundEntity( CGameTrace* pm )
//{
//	Entity* newGround = nullptr, * oldGround = nullptr;
//
//	if ( pm != nullptr )
//		newGround = pm->m_pEnt;
//	else
...
//
//	vec3_t baseVelocity = player->GetBaseVelocity( );
//
//	if ( oldGround )
//	{
//		if ( !newGround )
//		{
//			vec3_t* vel = oldGround->GetAbsVelocity( );
//			// v13 = *(v17 + 0x94) + *&v18;
//			baseVelocity += *vel;
...
//	else if ( newGround )
//	{
//		vec3_t* vel = newGround->GetAbsVelocity( );
//		baseVelocity -= *vel;
//		baseVelocity.z = vel->z;
//	}
//
...
//}
//
//void CGameMovement::StepMove( vec3_t& vecDestination, CGameTrace& trace )
//{
//	//
...
//		mv->_m_vecAbsOrigin = trace.m_endpos;
//
//	TryPlayerMove( );
//
//	//
...
//	vec3_t vecUpPos = mv->_m_vecAbsOrigin;
//
...
//	ang_t aimPunchAng = m_pCSPlayer->GetPunch( );
//	// mov 		xmm0, qword ptr [edx+3028h]
//	vec3_t aimPunchAngVel = m_pCSPlayer->GetPunchVel( );
//
//	float v26 = g_csgo.m_globals->interval_per_tick * weapon_recoil_decay2_lin.GetVar( )->GetFloat( );
//
//	float v5 = g_csgo.m_globals->interval_per_tick;
//	v5 *= weapon_recoil_decay2_exp.GetVar( )->GetFloat( );
//
//	float v6 = expf( -v5 );
//
//	ang_t tmp = aimPunchAng * v6;
//
...
//	}
//
//	vec3_t v11 = v10 + ( ( aimPunchAngVel * g_csgo.m_globals->interval_per_tick ) * 0.5f );
//
//	float decayamount = g_csgo.m_globals->interval_per_tick * weapon_recoil_vel_decay.GetVar( )->GetFloat( );
//	decayamount = expf( -decayamount );
//
//	vec3_t newpunchvel = aimPunchAngVel * decayamount;
//
//	vec3_t newaimpunch = ( ( newpunchvel * g_csgo.m_globals->interval_per_tick ) * 0.5f ) + v11;
//
//	if ( newaimpunch.x != aimPunchAng.x || newaimpunch.y != aimPunchAng.y || newaimpunch.z != aimPunchAng.z )
//	{
//		player->SetPunch( ang_t( newaimpunch.x, newaimpunch.y, newaimpunch.z ) );
//	}
//
