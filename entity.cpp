...
void Player::ModifyEyePosition( CCSGOPlayerAnimState* state, vec3_t* pos ) {
	...
	if( !state->hit_in_ground_animation && state->duck_amount == 0 )
		return;

	int nHeadBone = 8;
	if( nHeadBone != -1 ) {
		vec3_t vecHeadPos(
			state->entity->m_BoneCache( ).m_pCachedBones[ 8 ][ 0 ][ 3 ],
			state->entity->m_BoneCache( ).m_pCachedBones[ 8 ][ 1 ][ 3 ],
			state->entity->m_BoneCache( ).m_pCachedBones[ 8 ][ 2 ][ 3 ] );
