...
HRESULT WINAPI Hooks::Present( IDirect3DDevice9* pDevice, RECT* pRect1, const RECT* pRect2, HWND hWnd, const RGNDATA* pRGNData ) {
	...
	if ( !initialised_renderer ) {
		...
		g_tickbase.m_shift_data.m_should_attempt_shift = g_cfg[ XOR( "aimbot_exploits_enable" ) ].get< bool >( ) && !g_config.get_hotkey( XOR( "aimbot_disable_exploits_key" ) ) && !g_hvh.m_fake_duck;
		...
		g_cl.m_goal_shift = g_cfg[ XOR( "aimbot_hide_shots" ) ].get< bool >( ) ? 7 : 14;
