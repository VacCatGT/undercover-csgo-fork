...
public:
	...
	virtual bool EnumEntity( Entity* pHandleEntity ) = 0;
};
...
public:
	// sorry soar had to do it
	virtual int		GetPointContentsVtable( const vec3_t& vecAbsPosition, int contentsMask = MASK_ALL, Entity** ppEntity = NULL ) = 0;
	virtual int		GetPointContents_WorldOnly( const vec3_t& vecAbsPosition, int contentsMask = MASK_ALL ) = 0;
	virtual int		GetPointContents_Collideable( void* pCollide, const vec3_t& vecAbsPosition ) = 0;
	virtual void	ClipRayToEntityVirtual( const Ray& ray, unsigned int fMask, Entity* pEnt, CGameTrace* pTrace ) = 0;
	virtual void	ClipRayToCollideable( const Ray& ray, unsigned int fMask, void* pCollide, CGameTrace* pTrace ) = 0;
	virtual void	TraceRayVirtual( const Ray& ray, unsigned int fMask, ITraceFilter* pTraceFilter, CGameTrace* pTrace ) = 0;
	...
	virtual void	TraceRayAgainstLeafAndEntityList( const Ray& ray, ITraceListData* pTraceData, unsigned int fMask, ITraceFilter* pTraceFilter, CGameTrace* pTrace ) = 0;
	virtual void	SweepCollideable( void* pCollide, const vec3_t& vecAbsStart, const vec3_t& vecAbsEnd,
		const ang_t& vecAngles, unsigned int fMask, ITraceFilter* pTraceFilter, CGameTrace* pTrace ) = 0;

	virtual void	EnumerateEntities( const Ray& ray, bool triggers, IEntityEnumerator* pEnumerator ) = 0;
	virtual void	EnumerateEntities( const vec3_t& vecAbsMins, const vec3_t& vecAbsMaxs, IEntityEnumerator* pEnumerator ) = 0;
	virtual void* GetCollideable( Entity* pEntity ) = 0;
	...
	virtual bool PointOutsideWorld( const vec3_t& ptTest ) = 0;
	virtual int GetLeafContainingPoint( const vec3_t& ptTest ) = 0;
