...
namespace math {
	...
	bool Intersect(vec3_t start, vec3_t end, vec3_t a, vec3_t b, float radius);
	bool IntersectBB(vec3_t& start, vec3_t& end, vec3_t& min, vec3_t& max);
	...
	matrix3x4_t AngleMatrixToVec( const vec3_t angles );
	vec3_t VectorRotate( const vec3_t& in1, ang_t& in2 );
