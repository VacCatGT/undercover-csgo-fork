...
public:
	...
	__forceinline bool IsPlayer( ) {
		return util::get_method< bool( __thiscall* )( Entity* ) >( this, 157 )( this );
	}
...
public:
	...
	float velocity_z;                              //0x00D0 ??
	char pad_0x00D0[ 0xC ];                          //0x00DC
	...
	float time_since_in_air;                       //0x0110
	...
	__forceinline const char* GetWeaponPrefix( ) {
		...
		return ( ( GetWeaponPrefixFn )g_csgo.GetWeaponPrefix )( this );
	}
}; // size: 0x344
...
public:
	// netvars / etc.
	__forceinline float& m_flLaggedMovementValue( ) {
		return get< float >( g_entoffsets.m_flLaggedMovementValue );
	}
	...
	__forceinline float GetPlayerMaxSpeed( ) {
		return g_csgo.GetPlayerMaxSpeed( this );
	}

	__forceinline void InvalidateAnimationState( ) {
	
		( ( void( __thiscall* )( Player*, int ) ) g_csgo.InvalidateAnimState.as< uintptr_t >( ) )( this, 0x8 );
	
	}

	__forceinline int LookupSequence( const char* name ) {
		return ( ( int( __thiscall* )( Player*, const char* ) )g_csgo.LookupSequence )( this, name );
	}

	float GetLayerWeight( int layer )
	{
		C_AnimationLayer* pLayer = this->GetAnimationOverlay( layer );

		return pLayer->m_weight;
	}
	...
	vec3_t getBonePosition( int bone ) noexcept
	{
		if ( matrix3x4_t boneMatrices[ 256 ]; SetupBones( boneMatrices, 256, 256, 0.0f ) )
			return vec3_t{ boneMatrices[ bone ][ 0 ][ 3 ], boneMatrices[ bone ][ 1 ][ 3 ], boneMatrices[ bone ][ 2 ][ 3 ] };
		else
			return vec3_t{ };
	}
	...
	__forceinline int& GetStuckLast( ) {
		return get< int >( g_csgo.StuckLast );
	}

	__forceinline bool IsBot( )
	{
		DWORD resource = *( DWORD* )g_csgo.PlayerResource;
		if ( !resource )
			return false;

		DWORD resource2 = resource + g_csgo.IsBotPlayerResource;
		...
		DWORD val = util::get_method< OriginalFn >( ( void* )networkable, ( 0x28 / 4 ) )( networkable );

		typedef bool( __thiscall* OriginalFn2 )( DWORD, DWORD );
		bool isbot = util::get_method< OriginalFn2 >( ( void* )resource2, ( 0x18 / 4 ) )( resource2, val );

		return isbot;
	}

	__forceinline void AttachmentHelper()
	{
		static auto fn = pattern::find(g_csgo.m_client_dll, XOR("55 8B EC 83 EC 48 53 8B 5D 08 89 4D F4")).as<void(__thiscall*)(Player*, CStudioHdr*)>( );
		fn(this, this->GetModelPtr());
	};
	...
	__forceinline int SelectWeightedSequenceFromModifiers( int activity, void* modifiers, int count )
	{
		auto StudioHdr = this->m_studioHdr( );
		static const auto select_weighted_sequence_from_modifiers = ( int32_t( __thiscall* )( void*, void*, int32_t, const void*, int32_t ) ) pattern::find( g_csgo.m_server_dll, XOR( "55 8B EC 83 EC 2C ? ? ? 75 ? 8B ? 57 89" ) ).as< uintptr_t >( );
		static const auto find_mapping = ( void* ( __thiscall* )( void* ) ) pattern::find( g_csgo.m_client_dll, ( "55 8B EC 83 E4 F8 81 EC ? ? ? ? 53 56 57 8B F9 8B 17 83 BA ? ? ? ? ? 74 34 83 7F 04 00 75 2E 83 BA ? ? ? ? ? 75 04 33 C0 EB 0C 8B 0D ? ? ? ? 52 8B 01 FF 50 68" ) ).as< uintptr_t >( );

		if ( !find_mapping || !select_weighted_sequence_from_modifiers )
			return -1;

		return select_weighted_sequence_from_modifiers( find_mapping( StudioHdr ), StudioHdr, activity, modifiers, count );
	}

	__forceinline void GetSequenceLinearMotion( CStudioHdr* hdr, int sequence, float* poseparameter, vec3_t* out ) {
		using GetSequenceMoveDist_t = void( __fastcall* )( CStudioHdr*, int, float*, vec3_t* );
		( ( GetSequenceMoveDist_t )g_csgo.GetSequenceLinearMotion )( hdr, sequence, poseparameter, out );
		__asm add esp, 8
	}
...
public:
	...
	float GetSequenceCycleRate( CStudioHdr* hdr, int sequence ) {
		return util::get_method<float( __thiscall* )( void*, CStudioHdr*, int )>( this, GETSEQUENCECYCLERATE )( this, hdr, sequence );
	}
	...
	__forceinline unsigned GetPhysicsSolidMaskForEntity( ) {
		return util::get_method< unsigned( __thiscall* )( decltype( this ) ) >( this, PHYSICSSOLIDMASKFORENTITY )( this );
	}

	__forceinline float GetLayerSequenceCycleRate( C_AnimationLayer* state, int32_t layer ) {
		return util::get_method< float(__thiscall*)(void*, C_AnimationLayer*, int32_t)>( this, 222 )( this, state, layer);
		// not sure if this is right,
		// VFUNC(get_layer_sequence_cycle_rate(c_animation_layer* v1, int32_t v2), 222, float(__thiscall*)(void*, c_animation_layer*, int32_t), v1, v2)
	}
	...
	__forceinline C_AnimationLayer* GetAnimationOverlay(int i)
	{
	...
	__forceinline std::array<float, 24>& m_flPoseParameters( ) const {
		static int _m_flPoseParameter = g_entoffsets.m_flPoseParameter;
		return *(std::array<float, 24>*)((uintptr_t)this + _m_flPoseParameter);
	}

	__forceinline int WriteAnimLayer(C_AnimationLayer* src, C_AnimationLayer* dest)
	{
		int flags = 0;
		C_AnimationLayer* pStoredLayer = src;
		C_AnimationLayer* pLayer = dest;
		if (pLayer->m_flags != pStoredLayer->m_flags)
		{
			pLayer->m_flags = pStoredLayer->m_flags;
			flags |= ANIMATION_CHANGED;
		}

		if (pLayer->m_playback_rate != pStoredLayer->m_playback_rate)
		{
			pLayer->m_playback_rate = pStoredLayer->m_playback_rate;
			flags |= ANIMATION_CHANGED;
		}

		if (pLayer->m_weight_delta_rate != pStoredLayer->m_weight_delta_rate)
		{
			pLayer->m_weight_delta_rate = pStoredLayer->m_weight_delta_rate;
			flags |= ANIMATION_CHANGED;
		}

		if (pLayer->m_activty != pStoredLayer->m_activty)
		{
			pLayer->m_activty = pStoredLayer->m_activty;
			flags |= ANIMATION_CHANGED;
		}

		if (pLayer->m_weight != pStoredLayer->m_weight)
		{
			pLayer->m_weight = pStoredLayer->m_weight;
			flags |= ANIMATION_CHANGED;
		}

		if (pLayer->m_sequence != pStoredLayer->m_sequence)
		{
			pLayer->m_sequence = pStoredLayer->m_sequence;
			flags |= SEQUENCE_CHANGED;
			flags |= ANIMATION_CHANGED;
			flags |= BOUNDS_CHANGED;
		}

		if (pLayer->m_cycle != pStoredLayer->m_cycle)
		{
			pLayer->m_cycle = pStoredLayer->m_cycle;
			flags |= ANIMATION_CHANGED;
		}

		if (pLayer->m_order != pStoredLayer->m_order)
		{
			pLayer->m_order = pStoredLayer->m_order;
			flags |= BOUNDS_CHANGED;
		}
...
public:
	...
	__forceinline bool IsBaimable(bool extra_check = true) {
		return m_iItemDefinitionIndex() == Weapons_t::SCAR20
			|| m_iItemDefinitionIndex() == Weapons_t::AWP
			|| m_iItemDefinitionIndex() == Weapons_t::G3SG1;
	}
...
public:
	...
	__forceinline WeaponInfo* GetWpnData( ) {
		...
		public:
			virtual WeaponInfo* GetWpnData( unsigned ItemDefinitionIndex ) = 0;
		};
		static auto m_pWeaponSystem = pattern::find( g_csgo.m_client_dll, XOR( "8B 35 ? ? ? ? FF 10 0F B7 C0" ) ).add( 2 ).get< IWeaponSystem* >( );

		return m_pWeaponSystem->GetWpnData( this->m_iItemDefinitionIndex( ) );
	}
