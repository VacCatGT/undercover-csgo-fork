...
ImGuiStyle::ImGuiStyle( ) {
	...
	GrabMinSize = 0.f;            // Minimum width/height of a grab box for slider/scrollbar
...
bool ImGuiTextFilter::Draw( const char* label, float width ) {
	...
	bool value_changed = ImGui::InputText( true, label, InputBuf, IM_ARRAYSIZE( InputBuf ) );
	if ( width != 0.0f )
		ImGui::PopItemWidth( );
	if ( value_changed )
		Build( );
...
void ImGui::EndChild( bool shadow ) {
	...
	if ( window->ScrollbarY ) {
		...
		auto arrow = [ window ] ( float x, float y, bool normal, float clr = 1 ) {
			if ( normal ) {
				for ( auto i = 5; i >= 3; --i ) {
					auto offset = 5.f - float(i);

					window->DrawList->AddLine( ImVec2( x + offset, y + offset ), ImVec2( x + offset + nigga( i - offset, 0.f, 5.f ), y + offset ), ImColor( clr, clr, clr ) );
				}
			}
			else {
		...
		if ( window->Scroll.y > 1.0f ) {
			//window->DrawList->AddRectFilledMultiColor( min_top + ImVec2( 3, 0 ), max_top - ImVec2( 17, 0 ), ImColor( 0, 0, 0, 100 ), ImColor( 0, 0, 0, 100 ), ImColor( 0, 0, 0, 0 ), ImColor( 0, 0, 0, 0 ) );
			arrow( max_top.x - 30, max_top.y - 8, false );
		}
		if ( ( GetScrollMaxY( ) - window->Scroll.y ) > 1.f ) {
			//window->DrawList->AddRectFilledMultiColor( min_bot + ImVec2( 3, 0 ), max_bot - ImVec2( 17, 0 ), ImColor( 0, 0, 0, 0 ), ImColor( 0, 0, 0, 0 ), ImColor( 0, 0, 0, 100 ), ImColor( 0, 0, 0, 100 ) );
			arrow( max_bot.x - 30, max_bot.y - 10, true );
		}
	}
...
bool ImGui::BeginChildFrame( ImGuiID id, const ImVec2& size, ImGuiWindowFlags extra_flags ) {
	...
	ImGui::PushStyleColor( ImGuiCol_ChildWindowBg, ImVec4(31 / 255.f, 37 / 255.f, 43 / 255.f, 255 / 255.f) );
	ImGui::PushStyleVar( ImGuiStyleVar_ChildWindowRounding, 1.45 );
	ImGui::PushStyleVar( ImGuiStyleVar_WindowPadding, style.FramePadding );
	ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(56 / 255.f, 59 / 255.f, 62 / 255.f, 255 / 255.f)); 
	return ImGui::BeginChild( id, size, true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysUseWindowPadding | extra_flags | ImGuiWindowFlags_NoScrollbar);
}
...
bool ImGui::Begin( const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags ) {
	...
	if ( first_begin_of_the_frame ) {
		...
		ImRect title_bar_rect = window->TitleBarRect( );
		const float window_rounding = 1.45f;
		if ( window->Collapsed ) {
			// Draw title bar only
...
bool ImGui::ButtonExT(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int page, int in, bool border, ImColor clr) {
	...
	if (page == in) {
		...
		PushStyleColor(ImGuiCol_Text, ImVec4(180 / 255.f, 180 / 255.f, 180 / 255.f, 1));
		RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0.5, 0.5), &bb);
		PopStyleColor();
	}
...
bool ImGui::ButtonExT_Filled( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int page, int in, bool border, ImColor clr ) {
	ImGuiWindow* window = GetCurrentWindow( );
	if ( window->SkipItems )
		return false;
...
bool ImGui::ButtonT_Filled(const char* label, const ImVec2& size_arg, int page, int in, ImColor clr, bool border) {
	return ButtonExT_Filled(label, size_arg, 0, page, in, border, clr);
}
...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags ) {
	...
	ImVec2 pos = ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y + 10);
	if ( ( flags & ImGuiButtonFlags_AlignTextBaseLine ) && style.FramePadding.y < window->DC.CurrentLineTextBaseOffset ) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
		pos.y += window->DC.CurrentLineTextBaseOffset - style.FramePadding.y;
	ImVec2 size = CalcItemSize( size_arg, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f );
...
bool ImGui::SliderBehavior( const ImRect& frame_bb, ImGuiID id, const char* display_format, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags ) {
	...
	float grab_sz;
//	if (decimal_precision > 0)
		grab_sz = ImMin(0.f, slider_sz);
	//else
	//	grab_sz = ImMin(ImMax(1.0f * (slider_sz / ((v_min < v_max ? v_max - v_min : v_min - v_max))), style.GrabMinSize), slider_sz);  // Integer sliders, if possible have the grab size represent 1 unit
...
bool ImGui::SliderFloat( const char* label, float* v, float v_min, float v_max, const char* display_format, float power ) {
	...
	const ImVec2 label_size = CalcTextSize( label, NULL, true );
	const ImRect frame_bb( window->DC.CursorPos - ImVec2( -17, 2 ), window->DC.CursorPos + ImVec2( w + 17, label_size.y + style.FramePadding.y* 0.f - 11 ) );
...
bool ImGui::ShowCheckbox(const char* label) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::InputTextEx( const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data, bool long_input ) {
	ImGuiWindow* window = GetCurrentWindow( );
	if ( window->SkipItems )
		return false;
	...
	const ImGuiID id = window->GetID( label );
	const ImVec2 label_size = CalcTextSize( label, NULL, true );
	ImVec2 size = CalcItemSize( size_arg, 194, ( is_multiline ? GetTextLineHeight( ) * 8.0f : label_size.y ) + style.FramePadding.y * 2.0f ); // Arbitrary default of 8 lines high for multi-line
	const ImRect frame_bb( ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y + 18), ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y + 18) + size - ImVec2( long_input ? 0 : 23, 0 ) );
	const ImRect total_bb( frame_bb.Min, frame_bb.Max + ImVec2( label_size.x > 0.0f ? ( style.ItemInnerSpacing.x + label_size.x ) : 0.0f, 0.0f ) );
	...
	if ( !is_multiline ) {
		RenderFrame( frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), ImColor(56, 59, 62, 255), false, style.FrameRounding ); // fortnite
	...
	if ( label_size.x > 0 )
		RenderText( ImVec2( frame_bb.Min.x, frame_bb.Min.y - 18), label );
...
bool ImGui::InputText( bool x, const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data, bool long_input ) {
	IM_ASSERT( !( flags & ImGuiInputTextFlags_Multiline ) ); // call InputTextMultiline()
	return InputTextEx( label, buf, ( int )buf_size, ImVec2( x ? 217 : 179 + 15, 20 ), flags, callback, user_data, long_input );
}
...
bool ImGui::InputScalarEx( const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags ) {
	...
	if ( InputText( false,	"", buf, IM_ARRAYSIZE( buf ), extra_flags ) ) // PushId(label) + "" gives us the expected ID from outside point of view
...
bool ImGui::Hotkey( const char* label, int* k, const ImVec2& size_arg, int* type ) {
	...
	const ImGuiID id = window->GetID(label);
	const ImVec2 label_size = ImVec2(47, 0);
	...
	const bool user_clicked = hovered && io.MouseClicked[0];
	static bool is_keybind_selector_opened = false;
	...
	//	if(is_keybind_selector_opened)
		//	Combo("", keybind_type, "Always On\0Toggle\0Hold", 3);
	...
	ImGui::GetWindowDrawList()->AddRect(frame_bb.Min + ImVec2(16, 0) - ImVec2(0, 2), frame_bb.Min + ImVec2(36, 15) + ImVec2(16, 0) - ImVec2(0, 2), ImColor(56, 59, 62, 255), 1.45);
	RenderTextClipped(frame_bb.Min - ImVec2((KeyNamesSize.x / 2) - 32, -style.KeyBindOverride.y + 2) + ImVec2(2, 0), frame_bb.Max, buf_display, NULL, NULL);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImVec2 popup_size)
{
	...
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(22, 0) - ImVec2(0, 1), window->DC.CursorPos + ImVec2(w + 18, label_size.y + style.FramePadding.y * 2.0f - 1) + ImVec2(0, 1));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	...
	RenderTextClipped(frame_bb.Min + ImVec2(6, 1), value_bb.Max, preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
	...
	if (popup_open) {
		...
		else
		{
			...
			SetNextWindowPos(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y + 20), ImGuiCond_Always);
		}
		SetNextWindowSize(ImVec2(popup_size.x - 2, popup_size.y), ImGuiCond_Always);
...
bool ImGui::Combo( const char* label, int* current_item, bool( *items_getter )( void*, int, const char** ), void* data, int items_count, int height_in_items ) {
	...
	auto arrow = [ window ] ( float x, float y, float clr = 0.59 ) {
		for ( auto i = 5; i >= 2; --i ) {
			auto offset = 5.f - float(i);

			window->DrawList->AddLine( ImVec2( x + offset, y + offset ), ImVec2( x + offset + nigga( i - offset, 0.f, 5.f ), y + offset ), GetColorU32( ImVec4( clr, clr, clr, 1 ) ) );
...
//	ImGui::GetCurrentWindow()->DrawList()(bb_with_spacing.Min, bb_with_spacing.Max, ImColor(31, 37, 43), false, 0.0f);
...
bool ImGui::Selectable22( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg ) {
	ImGuiWindow* window = GetCurrentWindow( );
	if ( window->SkipItems )
		return false;
	...
	float w_draw = 100;
	ImVec2 size_draw( ( size_arg.x != 0 && !( flags & ImGuiSelectableFlags_DrawFillAvailWidth ) ) ? size_arg.x : w_draw, size_arg.y != 0.0f ? size_arg.y : size.y );
	ImRect bb_with_spacing( pos - ImVec2( 20, 0 ), pos + size_draw );
	if ( size_arg.x == 0.0f || ( flags & ImGuiSelectableFlags_DrawFillAvailWidth ) )
		bb_with_spacing.Max.x += window_padding.x;
...
bool ImGui::ListBoxHeader( const char* label, const ImVec2& size_arg ) {
	...
	ImVec2 frame_size = ImVec2( size.x + 17, ImMax( size.y, label_size.y ) );
	...
	BeginGroup( );
	if ( label_size.x > 0 )
		RenderText( ImVec2( frame_bb.Min.x + 2, frame_bb.Min.y - 20 ), label );

	BeginChildFrame( id, ImVec2(179 + 17, frame_bb.GetSize().y));
	return true;
}
...
bool ImGui::ListBoxHeader( const char* label, int items_count, int height_in_items ) {
	...
	ImGui::SetCursorPosY(ImGui::GetCursorPos().y + 13);
...
bool ImGui::ColorButton( const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size ) {
	...
	window->DrawList->AddRectFilled( bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), ImColor(56, 59, 62, 255), 1.45 ); // Color button are often in need of some sort of border

	ImVec4 col_without_alpha( col.x, col.y, col.z, 1.0f );
	float grid_step = ImMin( size.x, size.y ) / 2.99f;
//	if ( ( flags & ImGuiColorEditFlags_AlphaPreviewHalf ) && col.w < 1.0f ) {
		float mid_x = ( float )( int )( ( bb.Min.x + bb.Max.x ) * 0.5f + 0.5f );
		RenderColorRectWithAlphaCheckerboard( ImVec2( bb.Min.x, bb.Min.y ), bb.Max, GetColorU32( col ), grid_step, ImVec2( -grid_step, 0.0f ), 1.45f);
...
bool ImGui::ColorPicker4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags, const float* ref_col ) {
	...
	float bars_width = 16; // Arbitrary smallish width of Hue/Alpha picking bars
	float sv_picker_size = 149; // ImMax( bars_width * 1, CalcItemWidth( ) - ( alpha_bar ? 2 : 1 ) * ( bars_width + style.ItemInnerSpacing.x ) ); // Saturation/Value picking box
...
void ImGui::Columns( int columns_count, const char* id, bool border ) {
	...
	ImGuiColumnsFlags flags = ( border ? 0 : ImGuiColumnsFlags_NoBorder | ImGuiColumnsFlags_NoForceWithinWindow);
